...
- **`overrides.flatted` (`>=3.4.0`) and `overrides.serialize-javascript` (`>=7.0.3`) — reason recorded.** Both entries were added, alongside the pre-existing `ajv` override, in commit `33f3464` ("Phase 5 testing validation"), 2026-03-24 — before this log's Decision 1 was written, which is why no decision captured them. They pin two transitive dev-tool dependencies (pulled in via the Jest/Storybook/webpack toolchain — `flatted` through `istanbul`/`jest`-adjacent serialization, `serialize-javascript` through `webpack`) to versions patching known CVEs: `flatted` prototype-pollution/arbitrary-property-write below 3.2.7 (no CVE/GHSA currently open against this repo for it — `gh api .../dependabot/alerts` returns no `flatted` or `serialize-javascript` entries in any state), and `serialize-javascript` XSS via unsanitized regex/function serialization below 6.0.2. Neither ever surfaced as a GitHub/Dependabot alert on this repo (checked all 105 alerts, all states) — they were pinned proactively via `npm audit`'s advisory database rather than reactively via a tracked alert, which is why they read as undocumented. Confirmed still current and still resolving to patched versions in today's lockfile; kept as-is.
...
**Update (2026-09-23):** esbuild deferral resolved — PR #84 bumped `esbuild` to `0.28.2`; lockfile confirmed to resolve only that version tree-wide (no other esbuild version present). The `extract-zip` acceptance remains open (still blocked on `@lhci/cli` shipping on `lighthouse` >=13); see Decision 13 for the current full-audit tally (12: 6 low, 6 high, all dev-only, 0 in production).
